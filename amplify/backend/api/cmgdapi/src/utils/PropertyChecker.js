...
async function initializeTemplate(uniqueProperties, requiredProperties, multipleValuesProperties, propertyToRegex, properties) {
 ...
 */
function checkRegexAndAdditionalProperties(sample, propertyToRegex, multipleValuesProperties, errorAndWarningPool, properties) {
    ...
    for(const property in curation) {
        ...
        for(const value of values) {
            if(!regex || !regex.test(value)) {
                errorAndWarningPool.push(`error: wrong format: sample: ${sample.sampleId}, property: ${property}`);
                break;
            }
        }
    }
...
function checkRequiredness(sample, requiredProperties, errorAndWarningPool) {
    ...
    requiredProperties.forEach(property => {
        if(!(property in curation)) {
            errorAndWarningPool.push(`error: ${property} is required in sample: ${sample.sampleId}`);
            return false;
        }
    })
...
function checkUniqueness(sample, uniqueProperties, uniquePropertiesMap, errorAndWarningPool) {
    const curation = sample.curation;
        ...
        if(uniqueProperties.has(property)) {
            if(!uniquePropertiesMap.has(property)) {
                ...
                if(propertyValueSet.has(propertyValue)) {
                    errorAndWarningPool.push(`error: duplicated property: ${property} = ${propertyValue}, property must be unique`);
                } else {
...
module.exports = {
    ...
    async check(samples) {
        ...
        await initializeTemplate(uniqueProperties, requiredProperties, multipleValuesProperties, propertyToRegex, properties);

        samples.forEach(sample => {
            checkRegexAndAdditionalProperties(sample, propertyToRegex, multipleValuesProperties, errorAndWarningPool, properties);
            checkRequiredness(sample, requiredProperties, errorAndWarningPool);
            checkUniqueness(sample, uniqueProperties, uniquePropertiesMap, errorAndWarningPool);
        })
